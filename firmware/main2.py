...
def syncTime():
    try:
        ntptime.settime()
        util.syslog("Time", "Synced via NTP.")
    except Exception as e:
        util.syslog("Time", "Error getting NTP: {}".format(e))
    ...
    if (
        ...
        try:
            ...
            if (r.status_code == 200) and (
                ...
                with util.openFile("new_config.py") as f:
                    f.write(r.content)
                uos.rename("new_config.py", "config.py")
                util.syslog("OTA", "Updated config.py")
            else:
                util.syslog(
                    "OTA", "Hash mismatch, cowardly refusing to install update!"
                )
        except Exception as e:
            util.syslog("OTA", "Error getting updates: {}".format(e))
